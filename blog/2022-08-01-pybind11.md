...
wrapper.py的编写请移步: [实现第一个wrapper.py](https://xfyun.github.io/athena_website/docs/%E5%8A%A0%E8%BD%BD%E5%99%A8/Python%E6%8F%92%E4%BB%B6)
...
简而言之呢它是个什么玩意呢？ 比如有一个大段的数据快(音视频)，比如将它放在一个 array数组中，闲着我们需要用 numpy去加载它，如果这个array数组是原生的python list，那么这一过程必然是有拷贝过程的。但是如果设计了在c中利用buffer protocol设计出一个数据结构，这个数据结构传递到 numpy处理时，数据块可以直接指针转移，无需做数据拷贝，即可在numpy中处理该数据内存大块， 这在计算推理场景十分有用。
